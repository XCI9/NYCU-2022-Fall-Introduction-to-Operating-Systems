...
class ShortRemainTimeFirst {
    std::priority_queue<Process> m_waitingQueue;
  ...
  public:
    ShortRemainTimeFirst() {}

    void newProcessArrived(Process process) {
        // check preemptive
        if (process.timeLeft < m_currentProcess.timeLeft) {
    ...
    bool loop(std::vector<Process>& finishedProcess, const int currentTime) {
        if (m_isIdle && m_waitingQueue.empty())
            ...
            m_currentProcess = m_waitingQueue.top();
            m_waitingQueue.pop();
            m_isIdle = false;
        }
...
int main() {
    ...
    ShortRemainTimeFirst scheduler{};
